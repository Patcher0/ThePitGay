...
import static net.mizukilab.pit.util.rank.RankUtil.CONSTANT_BOT_NAME;
...
public class PublicUtil {
    ...
    public static void processActionBar(Player victim, Player damager, int damage,double finalDamage) {
        ...
        boolean npc = PlayerUtil.isNPC(victim);
        builder.append(npc ? CONSTANT_BOT_NAME : RankUtil.getPlayerColoredName(victim.getUniqueId()));
        boolean venom = !npc && PlayerUtil.isVenom(victim);
    ...
    public static void processActionBarWithSettingProvided(Player victim, Player damager, int damage,double finalDamage,PlayerProfile damagerProfile) {
        if (damagerProfile.isLoaded() && damagerProfile.getPlayerOption().getBarPriority() != PlayerOption.BarPriority.ENCHANT_ONLY) {
            processActionBar(victim, damager, damage,finalDamage);
        }
    }
        ...
        if (item instanceof CraftItemStack) {
            try {
                ...
                if (itemStack != null) {
                    ...
                    METHOD_ = lookup.unreflectVarHandle(handleField);
                }
                return itemStack;
