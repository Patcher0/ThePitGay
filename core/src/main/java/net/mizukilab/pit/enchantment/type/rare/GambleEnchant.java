...
public class GambleEnchant extends AbstractEnchantment implements IAttackEntity, Listener {
    ...
    public void handleAttackEntity(int enchantLevel, Player attacker, Entity target, double damage, AtomicDouble finalDamage, AtomicDouble boostDamage, AtomicBoolean cancel) {
        if (RandomUtil.hasSuccessfullyByChance(0.5)) {
            ...
            boolean isSelfSelected = RandomUtil.hasSuccessfullyByChance(0.5);
            if (isSelfSelected) {
                gamblePlayer = targetPlayer;
                attacker.sendMessage(CC.translate("&a&l赌徒! &7你赌赢了."));
            } else {
                attacker.sendMessage(CC.translate("&c&l赌徒! &7你赌输了."));
            }
